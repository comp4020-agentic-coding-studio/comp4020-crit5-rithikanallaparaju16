...
Working notes kept while building, moved out of `PROCESS.md` so the overview
there stays at the length a crit week asks for. Every prompt is verbatim, next
to the commits it produced. The two moments in `PROCESS.md` are curated out of
this; nothing here is a submission in its own right.
